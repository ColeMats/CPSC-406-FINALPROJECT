...
/*
Functions for computer generated moves
*/

std::string Game::getCompMove(void){
   // CODE ON HOLD BECAUSE BUGS
   /*
   Tree *moveTree = new Tree();
   TreeNode *comp_pieces = new TreeNode();
   std::vector<std::string> possPiece;
   for(int i =0; i<8;++i){
      for(int j = 0; j<8; ++j){
         char tempPiece = getPieceAtPosition(i,j);
   ...
   possPiece = comp_pieces->get_three_rand_keys();

   moveTree->setRoot(comp_pieces);

   for(int i = 0;i<3;++i){
      TreeNode *tempNode = new TreeNode();
      std::string moveFromTo = "";
      std::string moveTo = getPossibleComputerMoves(possPiece[i]);
      if(moveTo != "NA"){
         moveFromTo += possPiece[i];
         moveFromTo += "-";
         moveFromTo += moveTo;
         std::string type = "";
         type += comp_pieces->get_piece_locations().at(possPiece[i]);
         tempNode->set_piece_locations(possPiece[i],type);
         moveTree->addChild(comp_pieces, moveFromTo, tempNode);
      }
   }
   */
  std::vector<std::string> pieceLocations;
  for(int i =0; i<8;++i){
      for(int j = 0; j<8; ++j){
         char tempPiece = getPieceAtPosition(i,j);
         if((islower(tempPiece))&&(tempPiece != 'n') && (tempPiece != 'k') && (tempPiece != 'q')){
            std::string loc = "";
            loc += (char) (i+'A');
            loc += (char) (j + '1');
   ...
   int count = 0;
   while(count < 3){
      std::string totalMove = "";
      int index = std::rand() % pieceLocations.size();
      std::string movement = getPossibleComputerMoves(pieceLocations[index]);
      if( movement != "NA"){
         std::string pMove = pieceLocations[index];
         pMove += "-";
         pMove += movement;
   ...
   // random/smart move selection not done yet, 
   return possibleMoves[1];

}

std::string Game::getPossibleComputerMoves(std::string position){
   Position currP;
   currP.iColumn = position[0];
   currP.iRow = position[1];
   currP.iColumn = currP.iColumn - 'A';
   currP.iRow = currP.iRow - '1';

   std::string moveP = "";

   Position moveTo;
   char currPiece = getPieceAtPosition(currP.iRow, currP.iColumn);
   switch(currPiece){
      case 'p':
         moveP = getPawnMove(currP);
         break;
      case 'r':
         moveP = getRookMove(currP);
         break;
      case 'b':
         moveP = getBishopMove(currP);
         break;
      default:
         moveP = "NA";
         break;
...
// pawn movement generator -- limited movement
std::string Game::getPawnMove(Position currP){
   std::string moveTo = "";
   Position moveP;
   if(currP.iRow == 0){
      return "NA";
   }
   else if(currP.iRow == 6){ // if pawn at original location, automaticall move two spaces
      moveP.iRow = currP.iRow - 2;
      moveP.iColumn = currP.iColumn;
   }
   else if((currP.iColumn == 0) && (getPieceColor(getPieceAtPosition(currP.iRow-1, currP.iColumn + 1)) == WHITE_PIECE)){
      
      moveP.iRow = currP.iRow-1;
      moveP.iColumn = currP.iColumn + 1;
      
   }
   else if((currP.iColumn == 7) && (getPieceColor(getPieceAtPosition(currP.iRow-1, currP.iColumn - 1)) == WHITE_PIECE)){
      moveP.iRow = currP.iRow-1;
      moveP.iColumn = currP.iColumn - 1;
   }
   else{
      if(getPieceAtPosition(currP.iRow - 1, currP.iColumn) == EMPTY_SQUARE){
         moveP.iRow = currP.iRow-1;
         moveP.iColumn = currP.iColumn;
      }
      else{
   ...
   moveTo += (char) (moveP.iColumn + 'A');
   moveTo += (char) (moveP.iRow + '1');
   return moveTo;
}

// rook move generator -- limited movement (can move by only one square if at all)
std::string Game::getRookMove(Position currP){
   std::string moveTo = "";
   Position moveP;
   int row = currP.iRow;
   int col = currP.iColumn;
   if((row-1 >= 0) && (getPieceColor(getPieceAtPosition(row-1, col)) != BLACK_PIECE )){
      moveP.iRow = currP.iRow -1;
      moveP.iColumn = currP.iColumn;
   }
   else if((currP.iRow + 1 <= 7) && (getPieceColor(getPieceAtPosition(row+1, col)) != BLACK_PIECE )){
      moveP.iRow = currP.iRow +1;
      moveP.iColumn = currP.iColumn;
   }
   else if((currP.iColumn +1 <= 7) && (getPieceColor(getPieceAtPosition(currP.iRow, currP.iColumn+1)) != BLACK_PIECE )){
      moveP.iRow = currP.iRow;
      moveP.iColumn = currP.iColumn +1;
   }
   else if((currP.iColumn -1 >= 0) && (getPieceColor(getPieceAtPosition(currP.iRow, currP.iColumn-1)) != BLACK_PIECE )){
      moveP.iRow = currP.iRow;
      moveP.iColumn = currP.iColumn -1;
   }
   else{
...
// bishop move generator -- limited (no backwards movement, move one square at a time)
std::string Game::getBishopMove(Position currP){
   std::string moveTo = "";
   Position moveP;
   if((currP.iColumn + 1 <= 7) && (currP.iRow -1 >= 0)){
      if(getPieceColor(getPieceAtPosition(currP.iRow - 1, currP.iColumn + 1)) != BLACK_PIECE){
         moveP.iRow = currP.iRow - 1;
         moveP.iColumn = currP.iColumn + 1;
      }
   }
   else if((currP.iColumn - 1 <= 0) && (currP.iRow -1 >= 0)){
      if(getPieceColor(getPieceAtPosition(currP.iRow - 1, currP.iColumn - 1)) != BLACK_PIECE){
         moveP.iRow = currP.iRow - 1;
         moveP.iColumn = currP.iColumn + 1;
      }
